...
class UmTransaction 
	attr_accessor 	:key,		# Source key
					...
					def initialize
						...
						@lineitems = Array.new
						#@ip=ENV["REMOTE_ADDR"]
						@software="USAePay PHP API v. #{Usaepay_version}"
					end
					
					def addLine (sku, name, description, cost, qty, taxable)		
						@lineitems << {
						'sku' => sku,
						'name' => name,
					...
					def checkData 
						...
						command_array = ["cc:capture", "cc:refund", "refund", "check:refund","capture", "creditvoid", "quicksale", "quickcredit"]
					...
					if(!@pin.to_s.strip)
						...
						#	'<ClientIP xsi:type="xsd:string">' + @ip + '</ClientIP>' +
							...
							m = result.scan(/<AuthCode[^>]*>(.*)<\/AuthCode>/); @authcode = m.to_s if !m.empty? 
							m = result.scan(/<AvsResult[^>]*>(.*)<\/AvsResult>/); @avs_result = m.to_s if !m.empty?
							m = result.scan(/<AvsResultCode[^>]*>(.*)<\/AvsResultCode>/); @avs_result_code = m.to_s if !m.empty?
							m = result.scan(/<BatchRefNum[^>]*>(.*)<\/BetchRefNum>/); @batch = m.to_s if !m.empty?
							m = result.scan(/<CardCodeResult[^>]*>(.*)<\/CardCodeResult>/); @cvv2_result = m.to_s if !m.empty?
							m = result.scan(/<CardCodeResultCode[^>]*>(.*)<\/CardCodeResultCode>/); @cvv2_result_code = m.to_s if !m.empty?
							m = result.scan(/<ConversionRate[^>]*>(.*)<\/ConversionRate>/); @conversionrate = m.to_s if !m.empty?
							m = result.scan(/<ConvertedAmount[^>]*>(.*)<\/ConvertedAmount>/); @convertedamount = m.to_s if !m.empty?
							m = result.scan(/<ConvertedAmountCurrency[^>]*>(.*)<\/ConvertedAmountCurrency>/); @convertedamountcurrency = m.to_s if !m.empty?
							m = result.scan(/<Error[^>]*>(.*)<\/Error>/); @error = m.to_s if !m.empty?
							m = result.scan(/<ErrorCode[^>]*>(.*)<\/ErrorCode>/); @errorcode = m.to_s if !m.empty?
							m = result.scan(/<RefNum[^>]*>(.*)<\/RefNum>/); @refnum = m.to_s if !m.empty?
							m = result.scan(/<Result[^>]*>(.*)<\/Result>/); @result = m.to_s if !m.empty?
							m = result.scan(/<ResultCode[^>]*>(.*)<\/ResultCode>/); @resultcode = m.to_s if !m.empty?
							m = result.scan(/<VpasResultcode[^>]*>(.*)<\/VpasResultCode>/); @vpas_result_code = m.to_s if !m.empty?
